...
"""
Launch Isaac ROS Visual SLAM in RGBD mode with RTSP RGB (e.g. DJI drone) and
DA3METRIC-LARGE (TensorRT) depth.

Brings up:
  - static TF: camera_link -> camera_color_optical_frame
  - drone_node.py captures RTSP, publishes color/depth/camera_info
    (PYTHONPATH includes da3/src and tensorrt/)
  - visual_slam_node in a component container

Prerequisites: Reachable RTSP URL; TensorRT, cuda-python, OpenCV, rclpy;
depth_anything_3 importable from da3_src.
"""
...
_DEFAULT_RTSP_URL = 'rtsp://dji:dji@10.0.0.122:8554/streaming/live/1'


def _launch_drone_publisher(context, *args, **kwargs):
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument(
            'rtsp_url',
            default_value=_DEFAULT_RTSP_URL,
            description='RTSP URL for drone RGB stream (OpenCV FFmpeg backend)',
        ),
        DeclareLaunchArgument(
            'target_fps',
            default_value='30.0',
            description='Timer rate for grabbing latest RTSP frame and running depth inference',
        ),
        DeclareLaunchArgument(
            'da3_src',
    ...
    return launch.LaunchDescription([
        *args,
        static_tf_node,
        OpaqueFunction(function=_launch_drone_publisher),
